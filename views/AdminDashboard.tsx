...
import { AppData, User, UserRole, Sticker } from '../types';
...
type StickerEditorProps = {
  week: number;
  sticker: Sticker;
  onSave: (week: number, imageUrl: string, name: string, rarity: StickerRarity) => Promise<void>;
};

const RARITIES: StickerRarity[] = ['NORMAL', 'RUBY', 'EMERALD', 'OBSIDIAN', 'GOLD', 'DIAMOND'];

const StickerEditor: React.FC<StickerEditorProps> = ({ week, sticker, onSave }) => {
  const [isEditing, setIsEditing] = useState(false);
  const [url, setUrl] = useState(sticker.imageUrl || '');
  const [name, setName] = useState(
    sticker.name || (week >= 42 ? `Elo Supremo - Parte ${week - 40}` : `Semana ${week}`)
  );
  const [rarity, setRarity] = useState<StickerRarity>((sticker.rarity as StickerRarity) || 'NORMAL');
  ...
  useEffect(() => {
    ...
    setRarity((sticker.rarity as StickerRarity) || 'NORMAL');
  }, [isEditing, sticker.imageUrl, sticker.name, sticker.rarity, week]);

  const getRarityInfo = (r: StickerRarity) => {
    switch (r) {
      case 'RUBY':
        return { color: 'text-red-500', bg: 'bg-red-500', label: 'Rubi' };
      case 'EMERALD':
        return { color: 'text-emerald-500', bg: 'bg-emerald-500', label: 'Esmeralda' };
      case 'OBSIDIAN':
        return { color: 'text-slate-950', bg: 'bg-indigo-950', label: 'Obsidiana' };
      case 'GOLD':
        return { color: 'text-yellow-500', bg: 'bg-amber-500', label: 'Ouro' };
      case 'DIAMOND':
        return { color: 'text-cyan-500', bg: 'bg-cyan-500', label: 'Diamante' };
      default:
        return { color: 'text-indigo-400', bg: 'bg-indigo-600', label: 'Padrão' };
    }
  };
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center ml-4">
                  ...
                  <span
                    className={`text-[10px] font-black px-4 py-2 rounded-full text-white uppercase tracking-widest ${currentInfo.bg} border-2 border-indigo-950`}
                  >
                    {currentInfo.label}
                  </span>
                </div>
                    ...
                    return (
                      ...
                      >
                        <div className={`p-2 rounded-xl ${info.bg} text-white shadow-sm border-2 border-indigo-950`}>
                          {r === 'NORMAL' ? <ImageIcon size={18} /> : <Star size={18} />}
                        </div>
              ...
              <div className="flex gap-4 pt-2">
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    await onSave(week, '', defaultName, 'NORMAL');
...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ data, updateData }) => {
    ...
    return Array.from({ length: 45 }, (_, i) => {
      ...
      return (
        existing || {
          ...
          imageUrl: '',
          rarity: 'NORMAL' as StickerRarity
        }
      );
    });
  }, [data.stickers]);
  ...
  const handleUpdateSticker = useCallback(
    async (week: number, imageUrl: string, name: string, rarity: StickerRarity) => {
      ...
      const nextStickers = stickersForGrid.map(s => {
        ...
        return {
          ...
          imageUrl: imageUrl || '',
          rarity: (rarity || 'NORMAL') as StickerRarity
        } as Sticker;
      });

      await updateData({ stickers: nextStickers });
      setIsSyncing(false);
    },
    [stickersForGrid, updateData]
  );
              ...
              <div className="flex gap-4 pt-2">
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    // envia imageUrl vazio, backend deleta no banco
              ...
              <div className="flex gap-4 pt-2">
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    // manda vazio (backend vai DELETAR no banco)
                    await onSave(week, '', defaultName, StickerRarity.NORMAL);
...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ data, updateData }) => {
  ...
  const handleUpdateSticker = useCallback(
    async (week: number, imageUrl: string, name: string, rarity: StickerRarity) => {
      ...
      // mantém sempre os 45 no estado local, mas o backend vai deletar quando imageUrl vier vazio
      const nextStickers = stickersForGrid.map(s => {
        if (s.week !== week) return s;
        return {
          id: `sticker-${week}`,
