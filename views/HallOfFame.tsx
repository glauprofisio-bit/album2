...
import {
  Trophy, Star, Crown, Medal, ChevronLeft, Sparkles, Gem, UserCircle,
...
const HallOfFame: React.FC<HallOfFameProps> = ({ data, onClose }) => {
  ...
  const ranking = useMemo(() => {
    return data.students
      .map(student => {
        ...
        const epics = revealedStickers.filter(s => {
          const info = data.stickers.find(si => si.week === s.week);
          return info?.rarity === StickerRarity.DIAMOND || info?.rarity === StickerRarity.OBSIDIAN;
        }).length;

        let maxStreak = 0;
        let currentStreak = 0;
        for (let w = 1; w <= data.currentWeek; w++) {
          const st = stickers.find(x => x.week === w);
          if (st && st.liberada && !st.isFalta) {
            currentStreak++;
  ...
  const clubRares = useMemo(() => ranking.filter(s => s.epics > 0), [ranking]);
  const unstoppables = useMemo(
    () => ranking.filter(s => s.maxStreak >= 2).sort((a, b) => b.maxStreak - a.maxStreak),
    [ranking]
  );

  const showGeneralRanking = ranking.length >= 4; // evita “um aluno solto” quando tem poucos
  ...
  return (
    <div className="space-y-16 pb-24 animate-in fade-in slide-in-from-bottom-10 duration-700">
      ...
      <div className="text-center space-y-6 px-4">
        ...
        <p className="text-yellow-400 font-black uppercase tracking-[0.6em] text-[10px] md:text-xs animate-pulse">
        ...
        {topThree[0] && (
          <div className="order-1 md:order-2 flex flex-col items-center scale-110 md:scale-125 relative z-20">
            ...
            <div className="relative mb-10 group">
              ...
              <div className="absolute -top-8 -right-8 bg-yellow-400 text-indigo-950 w-20 h-20 rounded-full flex items-center justify-center font-black text-3xl border-[10px] border-indigo-950 shadow-2xl animate-pulse">
                1º
              </div>
            </div>
      ...
      <div className="max-w-5xl mx-auto px-4 py-10">
        <div className="bg-indigo-950/80 rounded-[4.5rem] border-[10px] border-indigo-950 p-12 relative overflow-hidden group club-card-shimmer">
          ...
          <div className="relative z-10 flex flex-col items-center gap-8">
            <div className="flex flex-wrap items-center justify-center gap-4 bg-black/50 px-10 py-4 rounded-full border-2 border-cyan-400 shadow-[0_0_40px_rgba(34,211,238,0.3)]">
              ...
              <span className="bg-cyan-400/20 text-cyan-200 border border-cyan-400/40 px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-widest">
                {clubRares.length} membros
              </span>
            </div>
            ...
            <div className="text-center space-y-2">
              ...
              <p className="text-white/40 text-[10px] font-black uppercase tracking-[0.2em] italic">
                Acesso para quem possui Diamante ou Obsidiana revelados
              </p>
            </div>
            ...
            <div className="flex flex-wrap justify-center gap-6 mt-6">
              {clubRares.length === 0 ? (
                ...
                clubRares.map(student => (
                  <div
                    key={student.id}
                    className="bg-white/5 backdrop-blur-2xl border-2 border-white/10 rounded-3xl px-8 py-6 flex items-center gap-5 hover:border-cyan-400/50 hover:bg-white/10 transition-all cursor-help relative group/item"
                  >
      ...
      <div className="max-w-5xl mx-auto px-4 py-10">
        <div className="bg-orange-950/90 rounded-[4.5rem] border-[10px] border-orange-500/30 p-12 relative overflow-hidden group shadow-[0_0_60px_rgba(249,115,22,0.1)]">
          <div className="relative z-10 flex flex-col items-center gap-8">
            <div className="flex flex-wrap items-center justify-center gap-4 bg-orange-600 px-10 py-4 rounded-full border-4 border-indigo-950 shadow-2xl animate-float">
              ...
              <span className="bg-black/20 text-white/80 border border-white/20 px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-widest">
                {unstoppables.length} elegíveis
              </span>
            </div>

            <div className="text-center space-y-2">
              <p className="text-orange-400 text-xs font-black uppercase tracking-[0.4em]">Elite da Constância</p>
              <p className="text-white/40 text-[10px] font-black uppercase tracking-[0.2em] italic">
                Maior sequência de semanas sem faltas
              </p>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 w-full mt-4">
              {unstoppables.length === 0 ? (
                <div className="col-span-full py-10 text-center text-white/10 font-black uppercase tracking-widest italic">
                  Aguardando os primeiros veteranos...
                </div>
              ) : (
                unstoppables.slice(0, 6).map(student => (
                  <div
                    key={student.id}
                    className="bg-white/5 border-2 border-orange-500/20 rounded-[2.5rem] p-6 flex items-center justify-between hover:bg-orange-600/10 transition-all"
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div className="text-left">
                        <p className="font-black text-white text-md uppercase italic tracking-tighter leading-none mb-1 truncate max-w-[120px]">
                          {student.name}
                        </p>
      ...
      {showGeneralRanking && (
        <div className="max-w-4xl mx-auto space-y-6 px-4">
          <div className="flex flex-col items-center gap-3 mb-6">
            <h3 className="font-black uppercase text-white/25 text-[10px] md:text-xs tracking-[0.6em] italic">
              Ranking Geral (4º em diante)
            </h3>
          ...
          {rest.map((student, idx) => (
            <div
              key={student.id}
              className="bg-white rounded-[2.5rem] p-6 border-[8px] border-indigo-950 shadow-[0_10px_0_0_rgba(30,27,75,1)] flex items-center justify-between hover:translate-x-3 transition-transform duration-300 group"
            >
              <div className="flex items-center gap-8">
                <span className="text-3xl font-black text-indigo-100 italic w-12 text-center group-hover:text-indigo-200 transition-colors">
                  #{idx + 4}
                </span>
      ...
      <div className="text-center pt-10 pb-20 opacity-30">
        <p className="text-[10px] font-black uppercase tracking-[0.5em] italic">
          Atualizado em Tempo Real • PEI Dr. Disnei
        </p>
      </div>
    </div>
  );
