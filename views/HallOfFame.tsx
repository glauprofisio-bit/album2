...
const HallOfFame: React.FC<HallOfFameProps> = ({ data, onClose }) => {
  ...
  const ranking = useMemo(() => {
    return data.students
      .map(student => {
        ...
        // Clube dos Raros: DIAMOND, GOLD, OBSIDIAN
        const rareCount = revealedStickers.filter(s => {
          const stickerInfo = data.stickers.find(si => si.week === s.week);
          return (
            stickerInfo?.rarity === StickerRarity.DIAMOND ||
            stickerInfo?.rarity === StickerRarity.GOLD ||
            stickerInfo?.rarity === StickerRarity.OBSIDIAN
          );
        }).length;
  ...
  return (
    <div className="space-y-16 pb-24 animate-in fade-in slide-in-from-bottom-10 duration-700">
          ...
          <div className="relative z-10 flex flex-col items-center gap-8">
            ...
            <div className="text-center space-y-2">
              ...
              <p className="text-white/40 text-[10px] font-black uppercase tracking-[0.2em] italic">
                Acesso exclusivo para quem possui Diamante, Ouro ou Obsidiana
              </p>
            </div>
        ...
        <div className="bg-orange-950/90 rounded-[4.5rem] border-[10px] border-orange-500/30 p-12 relative overflow-hidden group shadow-[0_0_60px_rgba(249,115,22,0.1)]">
          ...
          <div className="relative z-10 flex flex-col items-center gap-8">
            ...
            <div className="text-center space-y-2">
              ...
              <p className="text-white/40 text-[10px] font-black uppercase tracking-[0.2em] italic">Maior sequência de semanas sem faltas</p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 w-full mt-4">
              {unstoppables.length === 0 ? (
                ...
                unstoppables.slice(0, 6).map(student => (
                  <div key={student.id} className="bg-white/5 border-2 border-orange-500/20 rounded-[2.5rem] p-6 flex items-center justify-between hover:bg-orange-600/10 transition-all">
                    <div className="flex items-center gap-4">
                      <div className="relative">
                        <div className="w-14 h-14 rounded-2xl border-4 border-indigo-950 overflow-hidden bg-white shadow-lg flex-shrink-0">
                          {getAvatarUrl(student) ? <img src={getAvatarUrl(student)!} className="w-full h-full object-cover" /> : <UserCircle className="text-slate-200" size={32} />}
                        </div>
                      ...
                      <div className="text-left">
                        <p className="font-black text-white text-md uppercase italic tracking-tighter leading-none mb-1 truncate max-w-[140px]">{student.name}</p>
      ...
      {/* RANKING GERAL (AGORA FICA CLARO, NÃO PARECE UM ALUNO “SOLTO”) */}
      <div className="max-w-4xl mx-auto space-y-6 px-4">
        <div className="flex flex-col items-center gap-4 mb-10">
          <h3 className="font-black uppercase text-white/20 text-[10px] md:text-xs tracking-[0.8em] italic">
            Ranking Geral dos Colecionadores
          </h3>
          <div className="h-1 w-24 bg-white/10 rounded-full" />
          <p className="text-white/30 text-[10px] font-black uppercase tracking-widest">
            Inclui todos os alunos (inclusive o Top 3)
          </p>
        </div>

        {ranking.length === 0 ? (
          <p className="text-center text-white/20 font-black uppercase tracking-widest py-20">Nenhuma lenda detectada ainda...</p>
        ) : (
