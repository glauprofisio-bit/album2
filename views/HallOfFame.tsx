...
import { Trophy, Star, Crown, Medal, ChevronLeft, Sparkles, Gem, UserCircle, ShieldAlert, Flame, Zap } from 'lucide-react';
...
const HallOfFame: React.FC<HallOfFameProps> = ({ data, onClose }) => {
  ...
  const getRarityOfWeek = (week: number) => {
    const info = data.stickers.find(s => s.week === week);
    const raw = (info?.rarity ?? 'NORMAL') as any;
    return String(raw).toUpperCase();
  };

  const isEpic = (week: number) => {
    const r = getRarityOfWeek(week);
    return r === String(StickerRarity.DIAMOND) || r === String(StickerRarity.OBSIDIAN) || r === 'DIAMOND' || r === 'OBSIDIAN';
  };
  ...
  const ranking = useMemo(() => {
    return data.students
      .map(student => {
        ...
        // epics: considera quem "ganhou" (presença) ou revelou
        const epicEarnedCount = stickers.filter(s => (s.revelada || (s.liberada && !s.isFalta)) && isEpic(s.week)).length;

        // streak (sequência sem faltas)
        let maxStreak = 0;
        let currentStreak = 0;
        ...
        <div className="bg-indigo-950/80 rounded-[4.5rem] border-[10px] border-indigo-950 p-12 relative overflow-hidden group club-card-shimmer">
          ...
          <div className="relative z-10 flex flex-col items-center gap-8">
            ...
            <div className="text-center space-y-2">
              ...
              <p className="text-white/40 text-[10px] font-black uppercase tracking-[0.2em] italic">Acesso para quem ganhou Diamante ou Obsidiana</p>
            </div>
            ...
            <div className="bg-red-500/10 border-2 border-red-500/20 px-8 py-4 rounded-[2rem] flex items-center gap-3 mt-4">
              ...
              <p className="text-red-400 text-[9px] md:text-[10px] font-black uppercase tracking-widest leading-tight text-center italic">
                AVISO: AS IMAGENS DAS RARIDADES SÃO RESTRITAS AOS MEMBROS.<br />
                O MISTÉRIO FAZ PARTE DO CLUBE!
              </p>
            </div>
          </div>
        </div>
